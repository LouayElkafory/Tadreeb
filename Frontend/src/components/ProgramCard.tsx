...
export default function ProgramCard({ program }: { program: Program }) {
  ...
  return (
    <div className="flex flex-col p-5 rounded-2xl border border-soft-blue bg-white hover:border-primary hover:shadow-lg hover:-translate-y-0.5 transition-all">
      ...
      <h3 className="text-base font-bold text-deep-navy mb-1.5 leading-snug">{program.name}</h3>
      <p className="text-xs font-medium text-primary mb-2">{org?.shortName}</p>
      <p className="text-sm text-text-secondary leading-relaxed mb-4 flex-1">{program.description}</p>
      {program.skills && (
      ...
      <div className="flex items-center gap-2 mt-auto pt-1">
        ...
        <Link
          to="/chat"
          state={{ prefill: `احكيلي عن برنامج ${program.name} وشروط التقديم عليه.` }}
          ...
          aria-label="اسأل AI عن البرنامج"
        >
          <Sparkles size={14} />
        </Link>
      </div>
