  ...
  return (
    <div className="flex flex-col h-full bg-white">
      <div className="flex items-center justify-between px-4 py-4 border-b border-soft-blue shrink-0">
        <div>
          <h2 className="text-sm font-bold text-deep-navy">المصادر</h2>
          <p className="text-xs text-text-secondary mt-0.5">{sources.length} مصدر متاح</p>
        </div>
        {onClose && (
          <button
            onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {isLoading ? (
          ...
          <EmptyState
            ...
            title="مفيش مصادر لسه"
            description="المصادر المستخدمة في الإجابة هتظهر هنا بعد ما تسأل سؤال."
          />
        ) : (
          <div className="space-y-3">
            {sources.map((source) => (
